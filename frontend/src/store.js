...
export const isAdmin = derived(
  user,
  ($user) => $user?.user_roles?.includes("admin") || false
);
export const isOwner = derived(
  user,
  ($user) => $user?.user_roles?.includes("owner") || false
);
export const isFleet = derived(
  user,
  ($user) => $user?.user_roles?.includes("fleetmanager") || false
);

// Utility function to check if the user has any of the specified roles
export function hasAnyRole(...roles) {
  const current = get(user);
  return roles.some((r) => current?.user_roles?.includes(r));
}
